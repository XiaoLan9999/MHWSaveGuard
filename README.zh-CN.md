...
- userdata 列表改为固定宽度表格，并增加横向滚动条，避免文字挤在一起。
- 默认隐藏角色名候选扫描，因为 MHW 存档是二进制文件，强行扫描很容易出现乱码和误判。
...
MHW 的 `SAVEDATA1000` 是二进制存档文件。早期版本尝试从可打印字符串中扫描“角色名候选”，但很容易出现乱码和误判。当前版本默认隐藏该列，建议结合 Steam 昵称 / 状态、AccountID、remote 路径、存档修改时间和文件大小来判断。
